...
int main() {
    ...
    std::string hnsw1_path = "sub1_hnsw_1M_t80.bin"; 
    std::string hnsw2_path = "sub2_hnsw_1M_t80.bin";
    std::string hnsw3_path = "sub3_hnsw_1M_t80.bin"; 
    ...
    int share = num_base / 3;
    int remainder = num_base % 3;
    ...
    hnswlib::HierarchicalNSW<int>* hnsw2 = new hnswlib::HierarchicalNSW<int>(&space, share + remainder, M, ef_construction);
    hnswlib::HierarchicalNSW<int>* hnsw3 = new hnswlib::HierarchicalNSW<int>(&space, share, M, ef_construction);
    ...
    #pragma omp parallel
    {
        ...
        // }
        // #pragma omp for
        // for (size_t i = share; i < 2 * share + remainder; i++) {
        //     hnsw2->addPoint((void *)(base_vectors.data() + i * dim), (i - share));
        // }
        #pragma omp for
        for (size_t i = 2 * share + remainder; i < num_base; i++) {
            hnsw3->addPoint((void *)(base_vectors.data() + i * dim), (i - (2 * share + remainder)));
        }
    }
    ...
    float recall = test_topk(hnsw3, queries, groundtruth, num_queries, dim, k, search_threads);
