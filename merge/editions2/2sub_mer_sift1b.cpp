...
int main() {
    ...
    #pragma omp parallel 
    {
        #pragma omp for
        for (int id = 0; id < hnsw1_elements; id++) {
            std::priority_queue<std::pair<int, hnswlib::labeltype>> result = sub_hnsw2->searchKnn(sub_hnsw1->getDataByInternalId(id), add);
            while(result.size() != 0) {
        ...
        #pragma omp for
        for (int id = hnsw1_elements; id < max_elements; id++) {
            std::priority_queue<std::pair<int, hnswlib::labeltype>> result = sub_hnsw1->searchKnn(sub_hnsw2->getDataByInternalId(id - hnsw1_elements), add);
            while(result.size() != 0) {
                // if (graph_U0.getNeighbors(id).size() == 2 * M)
                //     graph_U0.getNeighbors(id).pop();
                graph_U0.addNeighbor(id, result.top().second, result.top().first);
