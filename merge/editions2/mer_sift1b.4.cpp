...
int dim = 128;   int max_elements = 5000000;
int M = 16;     int ef_construction = 200;      int ef_search = 55;
...
int main() {
    ...
    cout << "Sub graph loading time: " << timer.getElapsedTimeSeconds() << " seconds" << endl;
    ...
    for (int id = 0; id < max_elements; id++) {
        ...
        if (id < hnsw1_elements) {
            ...
            memcpy(merged_hnsw->getExternalLabeLp(id), sub_hnsw3->getExternalLabeLp(id - hnsw1_elements - hnsw2_elements), sizeof(hnswlib::labeltype));
            memcpy(merged_hnsw->getDataByInternalId(id), sub_hnsw3->getDataByInternalId(id - (hnsw1_elements + hnsw2_elements)), merged_hnsw->data_size_);
        }
    }
    ...
    // {
        ...
        for (int id = 0; id < hnsw3_elements; id++) {
            ...
            for (int j = 0; j < size; j++) {
                ...
                graph_U0.addNeighbor(id + (hnsw1_elements + hnsw2_elements), neighbor_id + (hnsw1_elements + hnsw2_elements), dist);
            }
        }   
    // }
    cout << "Level 0 graph_U0 time: " << timer.getElapsedTimeSeconds() << " seconds" << endl;

    graph_U0.printGraph(0, 0);
    ...
    // #pragma omp parallel 
        ...
        #pragma omp parallel for
        for (int id = 0; id < hnsw1_elements; id++) {
            std::priority_queue<std::pair<int, hnswlib::labeltype>> result = sub_hnsw3->searchKnn(sub_hnsw1->getDataByInternalId(id), add * 2);
            while(result.size()!= 0) {
        ...
        #pragma omp parallel for
        for (int id = 0; id < hnsw3_elements; id++) {
            std::priority_queue<std::pair<int, hnswlib::labeltype>> result = sub_hnsw1->searchKnn(sub_hnsw3->getDataByInternalId(id), add * 2);
            while(result.size()!= 0) {
                // if (graph_U0.getNeighbors(id).size() == 2 * M)
                //     graph_U0.getNeighbors(id).pop();
    ...
    std::string com_hnsw_path = "com_1M_1t.bin";
    ...
    cout << "Level 1 to max neighbors rewriting time: " << timer.getElapsedTimeSeconds() << " seconds" << endl;
    cout << endl;
    ...
    //         merged_hnsw->element_levels_[id + (hnsw1_elements + hnsw2_elements)] = level;
    //         if (level == 0)
    //             merged_hnsw->linkLists_[id + (hnsw1_elements + hnsw2_elements)] = nullptr;
    //         else
    //         {
    //             int size = merged_hnsw->size_links_per_element_ * level;
    //             merged_hnsw->linkLists_[id + (hnsw1_elements + hnsw2_elements)] = (char *) malloc(size);

    //             if (merged_hnsw->linkLists_[id + (hnsw1_elements + hnsw2_elements)] != nullptr) {
    //                 memcpy(merged_hnsw->linkLists_[id + (hnsw1_elements + hnsw2_elements)], sub_hnsw3->linkLists_[id], size);
    //                 for (int i = 1; i <= level; i++) {
    //                     hnswlib::linklistsizeint* data = merged_hnsw->get_linklist(id + (hnsw1_elements + hnsw2_elements), i);
    //                     int size = merged_hnsw->getListCount(data);
    //                     hnswlib::tableint* datal = (hnswlib::tableint*) (data + 1);
    //                     for (int j = 0; j < size; j++) {
    //                         datal[j] = datal[j] + (hnsw1_elements + hnsw2_elements);
    //                     }
    //                 }     
    //             }
    //             else
    ...
    std::string groundtruth_path = "/media/raid5/myt/gnd/idx_5M.ivecs";  
    ...
    // std::string mer_hnsw_path = "mer_1M_t80.add2loop1ori1max.bin";
