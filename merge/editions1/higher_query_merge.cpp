...
int main() {
    ...
    int iteration = 0;
    do {
        std::vector<std::unordered_set<size_t>> U(graph_U0.getElementsNum());
        for (size_t i = 0; i < graph_U0.getElementsNum(); i++) {
    ...
    // DistanceGraph g(1, M);
    // hnswlib::linklistsizeint* data_test = merged_hnsw->get_linklist(29, 1);
    // int size_test = merged_hnsw->getListCount(data_test);
    // hnswlib::tableint* datal_test = (hnswlib::tableint*) (data_test + 1);
    // for (int i = 0; i < size_test; i++) {
    //     float dist = merged_hnsw->fstdistfunc_(merged_hnsw->getDataByInternalId(29), merged_hnsw->getDataByInternalId(datal_test[i]), merged_hnsw->dist_func_param_);
    //     g.addNeighbor(0, datal_test[i],dist);
    // }
    // g.printGraph();

    // DistanceGraph g(1, M);
    // hnswlib::linklistsizeint* data_test = hnsw1->get_linklist(29, 1);
    // int size_test = hnsw1->getListCount(data_test);
    // hnswlib::tableint* datal_test = (hnswlib::tableint*) (data_test + 1);
    // for (int i = 0; i < size_test; i++) {
    //     float dist = hnsw1->fstdistfunc_(hnsw1->getDataByInternalId(29), hnsw1->getDataByInternalId(datal_test[i]), hnsw1->dist_func_param_);
    //     g.addNeighbor(0, datal_test[i],dist);
    // }
    // g.printGraph();

    // cout << hnsw1->element_levels_[5545] << endl;
