...
int dim = 128;   int maxElementsNum = 10000;
int M = 16;     int ef_construction = 200;

// Pretend reading data from dataset
std::uniform_real_distribution<> distrib_real;
std::mt19937 rng;  rng.seed(100);
...
void saveMergedIndex(int curElementsNum, int maxLevel, int enterpointNode)
{
    ...
    merged_hnsw1->maxlevel_ = maxLevel;
    merged_hnsw1->enterpoint_node_ = enterpointNode;
}
...
int main() 
{
    ...
    for (size_t i = 0; i < curElementsNum; i++) {
        if (i % 1000 == 0)
            cout << i << "\t";
    ...
    // Save index
    std::string hnsw_path1 = "hnsw1.bin";
    alg_hnsw1->saveIndex(hnsw_path1);
    delete alg_hnsw1;
    std::string hnsw_path2 = "hnsw2.bin";
    ...
    size_t line = M * 1.25;     size_t cutoff = M * 0.75;
    ...
    cout << "Loop time: " << timer.getElapsedTimeSeconds() << " seconds" << endl;
    cout << endl;
    ...
    // Print one-off hnsw graph for comparision
    ...
    for (size_t id = 0; id < curElementsNum; id++)
    {
        ...
        merged_hnsw1->setListCount(neighborSizePtr, M);
