...
int main() {
    ...
    // #pragma omp parallel 
    {
        ...
        for (int id = 0; id < hnsw2_elements; id++) {
            ...
            std::vector<std::pair<float, hnswlib::labeltype>> vec1;
            std::vector<std::pair<float, hnswlib::labeltype>> vec3;
            while (!result1.empty()) {
            ...
            for (int i = 0; i < add; i++) {
                ...
                hnswlib::tableint G3_node = vec3[i].second + (hnsw1_elements + hnsw2_elements);
                graph_U0.addNeighbor(id + hnsw1_elements, G1_node, vec1[i].first);
                graph_U0.addNeighbor(id + hnsw1_elements, G3_node, vec3[i].first);
            }

            for (int i = 0; i < add; i++) {
                hnswlib::tableint G1_node = vec1[i].second;
                for (int j = 0; j < add; j++) {
                    hnswlib::tableint neighbor = vec3[j].second + (hnsw1_elements + hnsw2_elements);
            ...
            for (int i = 0; i < add; i++) {
                hnswlib::tableint G3_node = vec3[i].second + (hnsw1_elements + hnsw2_elements);
                for (int j = 0; j < add; j++) {
                    hnswlib::tableint neighbor = vec1[i].second;
    ...
    cout << aver / (hnsw2_elements * add * add) << endl;
