...
int main() {
    ...
    #pragma omp parallel 
    {
        #pragma omp for
        for (int id = 0; id < hnsw2_elements; id++) {
            std::priority_queue<std::pair<float, hnswlib::labeltype>> result = sub_hnsw1->searchKnn(sub_hnsw2->getDataByInternalId(id), add);
            while(result.size() != 0) {
                // if (graph_U0.getNeighbors(id).size() == 2 * M)
                //     graph_U0.getNeighbors(id).pop();
                graph_U0.addNeighbor(id + hnsw1_elements, result.top().second, result.top().first);
                result.pop();
            }
        }
        #pragma omp for
        for (int id = 0; id < hnsw2_elements; id++) {
            std::priority_queue<std::pair<float, hnswlib::labeltype>> result = sub_hnsw3->searchKnn(sub_hnsw1->getDataByInternalId(id), add);
            while(result.size() != 0) {
                // if (graph_U0.getNeighbors(id).size() == 2 * M)
                //     graph_U0.getNeighbors(id).pop();
                graph_U0.addNeighbor(id + hnsw1_elements, result.top().second + (2 * hnsw1_elements), result.top().first);
                result.pop();
            }
        }
    }
        ...
        for (int u = 0; u < graph_U0.getElementsNum(); u++) {
            ...
            for (int i = 0; i < neighbors.size(); i++) {
                ...
                for (int j = i + 1; j < neighbors.size(); j++) {
                    int si = neighbors[i];
                    int sj = neighbors[j];
                    if ((si >= hnsw1_elements && si < 2 * hnsw1_elements) && (sj < hnsw1_elements || sj >= 2 * hnsw1_elements) ||
                        (sj >= hnsw1_elements && sj < 2 * hnsw1_elements) && (si < hnsw1_elements || si >= 2 * hnsw1_elements)) {
