...
int main() {
    ...
    float* data_full = new float[dim * cur_elements * 2];
    for (int i = 0; i < dim * cur_elements * 2; i++) {
        data_full[i] = distrib_real(rng);  
    }
    float* data = new float[dim * cur_elements];
    for (int i = dim * cur_elements; i < dim * cur_elements * 2; i++) {
        data[i - dim * cur_elements] = data_full[i];
    }
    
    omp_set_num_threads(4);

    std::string sub_hnsw1_path = "sub_hnsw3_12.5k_4p.bin";
    hnswlib::HierarchicalNSW<float>* sub_hnsw1 = new hnswlib::HierarchicalNSW<float>(&space, sub_hnsw1_path);        
    cout << "Sub hnsw1 loaded \t";
    std::string sub_hnsw2_path = "sub_hnsw4_12.5k_4p.bin";
    ...
    #pragma omp parallel 
    {
        #pragma omp for
        for (int id = 0; id < hnsw1_elements; id++) {
            std::priority_queue<std::pair<float, hnswlib::labeltype>> result = sub_hnsw2->searchKnn(data + id * dim, add);
            while(result.size() != 0) {
        ...
        for (int id = hnsw1_elements; id < cur_elements; id++) {
            std::priority_queue<std::pair<float, hnswlib::labeltype>> result = sub_hnsw1->searchKnn(data + id * dim, add);
            while(result.size() != 0) {
                if (graph_U0.getNeighbors(id).size() == 2 * M)
                    graph_U0.getNeighbors(id).pop();
