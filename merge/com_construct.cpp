...
int main() {
    ...
    const size_t num_base = 20000000;  
    ...
    std::string groundtruth_path = "/media/raid5/myt/gnd/idx_20M.ivecs";  
    std::string hnsw_path = "com_20M_16t.bin"; 

    // cout << "Saving path is " << hnsw_path << endl;
    ...
    hnswlib::HierarchicalNSW<int>* hnsw = new hnswlib::HierarchicalNSW<int>(&space, num_base, M, ef_construction);
    ...
    for (size_t i = 0; i < num_base; i++) {
        hnsw->addPoint((void *)(base_vectors.data() + i * dim), i);
    }
